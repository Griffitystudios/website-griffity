...
export default function MenuOverlay({ isOpen, onClose }: MenuOverlayProps) {
  ...
  return (
    <div
      ...
      className={`${poppins.className} fixed inset-0 z-50 flex flex-col lg:flex-row`}
    ...
    >
      ...
      <aside className="relative hidden flex-col bg-white px-12 py-10 lg:flex lg:w-[57%] xl:px-16">
        <Link
          href="/"
          onClick={onClose}
          className="flex select-none items-baseline tracking-tight"
        >
        ...
        <nav className="mt-14 flex flex-col gap-y-3 sm:gap-y-4 lg:mt-12">
          {NAV_ITEMS.map((item, i) => (
            <Link
              ...
              className="group w-fit text-3xl font-medium leading-tight text-white/90 transition-all duration-200 hover:translate-x-2 hover:text-white sm:text-4xl xl:text-[42px]"
            >
              {item.label}
            </Link>
          ))}
